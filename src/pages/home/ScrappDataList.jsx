...
import UpdateScrappData from '../../components/Update/Update';
import InsertShift from '../../components/InsertShift/InsertShift';
...
const ScrappDataList = ({ matricule }) => {
    ...
    const fetchTodayScrappData = async () => {
        ...
        try {
            const responseShift = await axios.get('http://localhost:5062/api/ScrappDataShift/Today');
            setShiftData(responseShift.data.scrappDataShift || []);
        } catch (error) {
    ...
    .map((data, index) => (
        <div key={index} className="shift-item">
            ...
            {showInsertShiftForm && <InsertShift onInsertSuccess={handleInsertShiftSuccess} onCancel={handleCancel} />}
